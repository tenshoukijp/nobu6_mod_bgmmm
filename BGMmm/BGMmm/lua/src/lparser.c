...
typedef struct BlockCnt {
	...
	int continuelist;  /* list of jumps to the loop's test */ // Continue‚Ì’Ç‰Á
...
static void enterblock (FuncState *fs, BlockCnt *bl, lu_byte isbreakable) {
	...
	bl->continuelist = NO_JUMP; // Continue‚Ì’Ç‰Á
...
static UnOpr getunopr (int op) {
	...
	case '#': return OPR_LEN;
    case '~': return OPR_BNOT; // BitWise—p
...
//    case '\\': return OPR_INTDIV;
    case TK_IDIV: return OPR_INTDIV;
	// ‚±‚±‚Ü‚Å
...
static void assignment (LexState *ls, struct LHS_assign *lh, int nvars) {
	...
	if (testnext(ls, ',')) {  /* assignment -> `,' primaryexp assignment */
		...
		int nexps;
		luaX_next(ls); /* consume `=' token. for compound‰‰ŽZ */
		nexps = explist1(ls, &e);
		if (nexps != nvars) {
...
static void whilestat (LexState *ls, int line) {
	...
	luaK_patchlist(fs, bl.continuelist, whileinit);  /* continue goes to start, too */ // Continue•¶‚Ì’Ç‰Á‚Ì‚½‚ß
...
static void repeatstat (LexState *ls, int line) {
	...
	chunk(ls);
	luaK_patchtohere(fs, bl1.continuelist); // Continue•¶‚Ì’Ç‰Á‚Ì‚½‚ß
...
static void forbody (LexState *ls, int base, int line, int nvars, int isnum) {
	...
	luaK_patchtohere(fs, bl.previous->continuelist);	/* continue, if any, jumps to here */ // Continue•¶‚Ì’Ç‰Á‚Ì‚½‚ß
...
static int statement (LexState *ls) {
	...
	case TK_CONTINUE: {  /* stat -> continuestat */ // Continue•¶‚Ì’Ç‰Á‚Ì‚½‚ß
